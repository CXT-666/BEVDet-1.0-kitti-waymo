...
def _test_pred(results_path):
    ...
    nusc = NuScenesMars(version='v1.0-mini', dataroot='data/nuscenes')
    nusc_exp = NuScenesExplorerMars(nusc)
    samples = nusc.sample
    samples[0]
    lidar_names = ['LIDAR_TOP']
    ...
    selected_keys = list(results_dict.keys())[0:80:4]
    i = 0
    for sample_token in selected_keys:
        ...
        selected_cam_front_token = selected_sample['data']['CAM_FRONT']
        # selected_cam_front_left_token = selected_sample['data']['CAM_FRONT_LEFT']
        # selected_cam_front_right_token = selected_sample['data']['CAM_FRONT_RIGHT']
        # selected_cam_back_token = selected_sample['data']['CAM_BACK']
        # selected_cam_back_left_token = selected_sample['data']['CAM_BACK_LEFT']
        # selected_cam_back_right_token = selected_sample['data']['CAM_BACK_RIGHT']
        nusc_exp.render_sample_pred(selected_cam_front_token, results_dict[sample_token], out_path='./vis/bevdet/PD/'+str(i))
        nusc_exp.render_sample_data(selected_cam_front_token, out_path='./vis/bevdet/GT/'+str(i))
        # nusc_exp.render_sample_pred(selected_cam_front_left_token, results_dict[sample_token])
        # nusc_exp.render_sample_pred(selected_cam_front_right_token, results_dict[sample_token])
        # nusc_exp.render_sample_pred(selected_cam_back_token, results_dict[sample_token])
        # nusc_exp.render_sample_pred(selected_cam_back_left_token, results_dict[sample_token])
        # nusc_exp.render_sample_pred(selected_cam_back_right_token, results_dict[sample_token])
        i += 1


    
if __name__ == '__main__':
    _test_pred('results/json/pts_bbox_bevdet/results_nusc.json')