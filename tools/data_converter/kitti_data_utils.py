...
def get_waymo_image_info(path,
    ...
    def map_func(idx):
        ...
        Trv2c_dy={
            'CAM_FRONT' : Tr_velo_to_cam_0,
        'CAM_FRONT_LEFT' : Tr_velo_to_cam_1,
        'CAM_FRONT_RIGHT': Tr_velo_to_cam_2,
             'SIDE_LEFT': Tr_velo_to_cam_3,
            'SIDE_RIGHT': Tr_velo_to_cam_4,
        }

        R0_rect = R0_rect.astype(np.float32)
        Tr_velo_to_cam_0 = Tr_velo_to_cam_0.astype(np.float32)
        Tr_velo_to_cam_1 = Tr_velo_to_cam_1.astype(np.float32)
        Tr_velo_to_cam_2 = Tr_velo_to_cam_2.astype(np.float32)
        Tr_velo_to_cam_3 = Tr_velo_to_cam_3.astype(np.float32)
        Tr_velo_to_cam_4 = Tr_velo_to_cam_4.astype(np.float32)
        ...
        for cam in camera_types:
                ...
                image_path = get_image_path(
                    ...
                    training,
                    relative_path,
                    info_type=duiyingguanxi[cam],
                    use_prefix_id=True)
                cam_info['data_path'] = os.path.join(path,image_path)
                camera2lidar = np.linalg.inv(Trv2c_dy[cam])
                cam_info['sensor2lidar_rotation']=camera2lidar[:3, :3]
                cam_info['sensor2lidar_translation']=camera2lidar[:3, 3]
                cam_info['cam_intrinsic']=intrincs_dy[cam]
                if with_imageshape:
                    if relative_path:
                        img_path = str(root_path / image_path)
                    cam_info['image_shape'] = np.array(
                        io.imread(img_path).shape[:2], dtype=np.int32)
